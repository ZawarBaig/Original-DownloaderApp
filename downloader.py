...
class MrBaigDownloader:
    ...
    def setup_ui(self):
        ...
        # Developer Text (Top Left, below header)
        dev_label = tk.Label(self.root, text="Developed By Zawar Baig", bg=self.bg_color, fg="#7F8C8D", font=("Segoe UI", 9, "bold italic"))
        ...
        # Video Info (Thumbnail and Title) Area
        ...
        self.tree.column("Action", width=140) # Clickable Download Column
        
        self.tree.tag_configure('oddrow', background="#F9F9F9")
        self.tree.tag_configure('evenrow', background="#FFFFFF")
        # Style the action column text to look like a blue link
        self.tree.tag_configure('action_text', foreground=self.accent_color, font=("Segoe UI", 9, "bold"))
        ...
        # Bind clicking to the treeview to detect clicks on the Action column
    ...
    def load_thumbnail(self, thumb_url, title):
        try:
            ...
            # Resize it keeping aspect ratio (e.g., max width 120, height 80)
            im.thumbnail((120, 80))
    ...
    def fetch_formats(self, url):
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                # Try to load thumbnail and title
                ...
                # Add Best option with the new Action column
                formatted_list.append(("best", "mp4/mkv", "Best Quality", "Auto-merges best video and audio", "Auto", "📥 Click to Download"))
    ...
    def update_treeview(self, formats):
        for index, item in enumerate(formats):
            ...
            # Insert item and combine row tag with action_text tag for the blue download text look
            self.tree.insert("", "end", values=item, tags=(tag, 'action_text'))
            
        self.status_var.set("Formats loaded. Click '📥 Click to Download' in any row to start.")

    def on_tree_click(self, event):
        # Identify what part of the treeview was clicked
        region = self.tree.identify_region(event.x, event.y)
        if region != "cell": 
            return
            
        # Identify which column was clicked (#6 is the Action column)
        column = self.tree.identify_column(event.x)
        if column == '#6':
            item_id = self.tree.identify_row(event.y)
            if item_id:
                # Get the format ID from the first column of the clicked row
